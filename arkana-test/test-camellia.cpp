...
TYPED_TEST_P(CamelliaTest, rfc3713_test_vectors)
{
    ...
    // rfc3713 test vector
    {
        ...
        test_vector(TypeParam::camellia128_ecb_encrypt_context_t(key), plain, cipher);
        test_vector(TypeParam::camellia128_ecb_decrypt_context_t(key), cipher, plain);
    }
    ...
    // rfc3713 test vector
    {
        ...
        test_vector(TypeParam::camellia192_ecb_encrypt_context_t(key), plain, cipher);
        test_vector(TypeParam::camellia192_ecb_decrypt_context_t(key), cipher, plain);
    }
    ...
    // rfc3713 test vector
    {
        ...
        test_vector(TypeParam::camellia256_ecb_encrypt_context_t(key), plain, cipher);
        test_vector(TypeParam::camellia256_ecb_decrypt_context_t(key), cipher, plain);
    }
}

TYPED_TEST_P(CamelliaTest, rfc5528_test_vectors)
{
    auto test_vector = [](auto&& context, auto input, auto expected)
    {
        decltype(expected) buffer{};
...
TYPED_TEST_P(CamelliaTest, ecb_benchmark128)
{
    ...
    auto& source = TestFixture::source_for_benchmark();
    auto buffer = source;
    TypeParam::camellia128_ecb_encrypt_context_t(key)->process_blocks(buffer.data(), buffer.data(), buffer.size());
    TypeParam::camellia128_ecb_decrypt_context_t(key)->process_blocks(buffer.data(), buffer.data(), buffer.size());
    EXPECT_EQ(source, buffer);
}

TYPED_TEST_P(CamelliaTest, ecb_benchmark256)
{
    auto key = 0x01'23'45'67'89'ab'cd'ef'fe'dc'ba'98'76'54'32'10'00'11'22'33'44'55'66'77'88'99'aa'bb'cc'dd'ee'ff_byte_array;
    auto& source = TestFixture::source_for_benchmark();
    auto buffer = source;
    TypeParam::camellia256_ecb_encrypt_context_t(key)->process_blocks(buffer.data(), buffer.data(), buffer.size());
    TypeParam::camellia256_ecb_decrypt_context_t(key)->process_blocks(buffer.data(), buffer.data(), buffer.size());
    EXPECT_EQ(source, buffer);
}

TYPED_TEST_P(CamelliaTest, ctr_benchmark128)
{
    auto key = 0x01'23'45'67'89'ab'cd'ef'fe'dc'ba'98'76'54'32'10_byte_array;
    ...
    auto& source = TestFixture::source_for_benchmark();
    auto buffer = source;
    TypeParam::camellia128_ctr_context_t(key, iv, nonce)->process_bytes(buffer.data(), buffer.data(), 0, buffer.size());
    TypeParam::camellia128_ctr_context_t(key, iv, nonce)->process_bytes(buffer.data(), buffer.data(), 0, buffer.size());
    EXPECT_EQ(source, buffer);
}

TYPED_TEST_P(CamelliaTest, ctr_benchmark256)
{
    auto key = 0x01'23'45'67'89'ab'cd'ef'fe'dc'ba'98'76'54'32'10'00'11'22'33'44'55'66'77'88'99'aa'bb'cc'dd'ee'ff_byte_array;
    auto iv = 0x00'00'00'00'00'00'00'00_byte_array;
    auto nonce = 0x00'00'00'30_byte_array;
    auto& source = TestFixture::source_for_benchmark();
    auto buffer = source;
    TypeParam::camellia256_ctr_context_t(key, iv, nonce)->process_bytes(buffer.data(), buffer.data(), 0, buffer.size());
    TypeParam::camellia256_ctr_context_t(key, iv, nonce)->process_bytes(buffer.data(), buffer.data(), 0, buffer.size());
    EXPECT_EQ(source, buffer);
}

REGISTER_TYPED_TEST_CASE_P(CamelliaTest, rfc3713_test_vectors, rfc5528_test_vectors, ecb_benchmark128, ecb_benchmark256, ctr_benchmark128, ctr_benchmark256);

struct ia32_impl
{
    static auto camellia128_ecb_encrypt_context_t(const key_128bit_t& key) { return create_ecb_encrypt_context_ia32(&key); }
    static auto camellia192_ecb_encrypt_context_t(const key_192bit_t& key) { return create_ecb_encrypt_context_ia32(&key); }
    static auto camellia256_ecb_encrypt_context_t(const key_256bit_t& key) { return create_ecb_encrypt_context_ia32(&key); }
    static auto camellia128_ecb_decrypt_context_t(const key_128bit_t& key) { return create_ecb_decrypt_context_ia32(&key); }
    static auto camellia192_ecb_decrypt_context_t(const key_192bit_t& key) { return create_ecb_decrypt_context_ia32(&key); }
    static auto camellia256_ecb_decrypt_context_t(const key_256bit_t& key) { return create_ecb_decrypt_context_ia32(&key); }
    static auto camellia128_ctr_context_t(const key_128bit_t& key, const ctr_iv_t& iv, const ctr_nonce_t& nonce) { return create_ctr_context_ia32(&key, &iv, &nonce); }
    static auto camellia192_ctr_context_t(const key_192bit_t& key, const ctr_iv_t& iv, const ctr_nonce_t& nonce) { return create_ctr_context_ia32(&key, &iv, &nonce); }
...
struct avx2_impl
{
    static auto camellia128_ecb_encrypt_context_t(const key_128bit_t& key) { return create_ecb_encrypt_context_avx2(&key); }
    static auto camellia192_ecb_encrypt_context_t(const key_192bit_t& key) { return create_ecb_encrypt_context_avx2(&key); }
    static auto camellia256_ecb_encrypt_context_t(const key_256bit_t& key) { return create_ecb_encrypt_context_avx2(&key); }
    static auto camellia128_ecb_decrypt_context_t(const key_128bit_t& key) { return create_ecb_decrypt_context_avx2(&key); }
    static auto camellia192_ecb_decrypt_context_t(const key_192bit_t& key) { return create_ecb_decrypt_context_avx2(&key); }
    static auto camellia256_ecb_decrypt_context_t(const key_256bit_t& key) { return create_ecb_decrypt_context_avx2(&key); }
    static auto camellia128_ctr_context_t(const key_128bit_t& key, const ctr_iv_t& iv, const ctr_nonce_t& nonce) { return create_ctr_context_avx2(&key, &iv, &nonce); }
    static auto camellia192_ctr_context_t(const key_192bit_t& key, const ctr_iv_t& iv, const ctr_nonce_t& nonce) { return create_ctr_context_avx2(&key, &iv, &nonce); }
...
{
    static auto camellia128_ecb_encrypt_context_t(const key_128bit_t& key) { return create_ecb_encrypt_context_avx2aesni(&key); }
    static auto camellia192_ecb_encrypt_context_t(const key_192bit_t& key) { return create_ecb_encrypt_context_avx2aesni(&key); }
    static auto camellia256_ecb_encrypt_context_t(const key_256bit_t& key) { return create_ecb_encrypt_context_avx2aesni(&key); }
    static auto camellia128_ecb_decrypt_context_t(const key_128bit_t& key) { return create_ecb_decrypt_context_avx2aesni(&key); }
    static auto camellia192_ecb_decrypt_context_t(const key_192bit_t& key) { return create_ecb_decrypt_context_avx2aesni(&key); }
    static auto camellia256_ecb_decrypt_context_t(const key_256bit_t& key) { return create_ecb_decrypt_context_avx2aesni(&key); }
    static auto camellia128_ctr_context_t(const key_128bit_t& key, const ctr_iv_t& iv, const ctr_nonce_t& nonce) { return create_ctr_context_avx2aesni(&key, &iv, &nonce); }
    static auto camellia192_ctr_context_t(const key_192bit_t& key, const ctr_iv_t& iv, const ctr_nonce_t& nonce) { return create_ctr_context_avx2aesni(&key, &iv, &nonce); }
