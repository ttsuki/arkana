...
namespace arkana::camellia
{
    ...
    static inline std::unique_ptr<ecb_context_t> make_avx2_ecb_context(key_t k, enc_t e)
    {
        ...
        public:
            ...
            explicit ecb_context_impl_t(key_t key, enc_t enc)
                : key_vector_(avx2::impl::generate_key_vector(key, enc))
            {
            }

            void process_blocks(void* dst, const void* src, size_t length) override
            {
                return avx2::impl::process_blocks_ecb(dst, src, length, key_vector_);
            }
        };
    ...
    static inline std::unique_ptr<ctr_context_t> make_avx2_ctr_context(key_t k, ctr_t ctr)
    {
        ...
        public:
            ...
            explicit ctr_context_impl_t(key_t key, ctr_t ctr)
                : key_vector_(avx2::impl::generate_key_vector(key, true_t{}))
            ...
            {
                return avx2::impl::process_bytes_ctr(dst, src, position, length, key_vector_, ctr_provider_);
            }
        };
    ...
    std::unique_ptr<ctr_context_t> create_ctr_context_avx2(const key_128bit_t* key, const ctr_iv_t* iv, const ctr_nonce_t* nonce) { return make_avx2_ctr_context(*key, avx2::impl::generate_rfc5528_ctr_provider(*iv, *nonce)); }
    std::unique_ptr<ctr_context_t> create_ctr_context_avx2(const key_192bit_t* key, const ctr_iv_t* iv, const ctr_nonce_t* nonce) { return make_avx2_ctr_context(*key, avx2::impl::generate_rfc5528_ctr_provider(*iv, *nonce)); }
    std::unique_ptr<ctr_context_t> create_ctr_context_avx2(const key_256bit_t* key, const ctr_iv_t* iv, const ctr_nonce_t* nonce) { return make_avx2_ctr_context(*key, avx2::impl::generate_rfc5528_ctr_provider(*iv, *nonce)); }
}
