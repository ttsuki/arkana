...
namespace arkana::sha2
{
    ...
    namespace impl
    {
        inline namespace functions
        {
            ...
            static inline constexpr void compress(vector_t<T>& vec, T ki, T wi) noexcept
            {
                T a = vec[(0 - i) & 7];
                T b = vec[(1 - i) & 7];
                T c = vec[(2 - i) & 7];
            ...
            template <
                ...
                int S00, int S01, int S02, int S10, int S11, int S12>
            static inline constexpr void process_chunk_impl(vector_t<T>& vec, chunk_t<T> input) noexcept
            {
            ...
            template <size_t digest_bits, class T> static void process_chunk(vector_t<T>& v, const chunk_t<T>& input);
            template <> inline constexpr void process_chunk<224, uint32_t>(vector_t<uint32_t>& v, const chunk_t<uint32_t>& input) { return process_chunk_impl<uint32_t, round_constants_32, 64, 7, 18, 3, 17, 19, 10, 2, 13, 22, 6, 11, 25>(v, input); }
            template <> inline constexpr void process_chunk<256, uint32_t>(vector_t<uint32_t>& v, const chunk_t<uint32_t>& input) { return process_chunk_impl<uint32_t, round_constants_32, 64, 7, 18, 3, 17, 19, 10, 2, 13, 22, 6, 11, 25>(v, input); }
            template <> inline constexpr void process_chunk<384, uint64_t>(vector_t<uint64_t>& v, const chunk_t<uint64_t>& input) { return process_chunk_impl<uint64_t, round_constants_64, 80, 1, 8, 7, 19, 61, 6, 28, 34, 39, 14, 18, 41>(v, input); }
            template <> inline constexpr void process_chunk<512, uint64_t>(vector_t<uint64_t>& v, const chunk_t<uint64_t>& input) { return process_chunk_impl<uint64_t, round_constants_64, 80, 1, 8, 7, 19, 61, 6, 28, 34, 39, 14, 18, 41>(v, input); }

            template <size_t digest_bits, class T> static vector_t<T> initial_vector();
            template <> inline constexpr vector_t<uint32_t> initial_vector<224, uint32_t>() { return {0xc1059ed8, 0x367cd507, 0x3070dd17, 0xf70e5939, 0xffc00b31, 0x68581511, 0x64f98fa7, 0xbefa4fa4}; }
            template <> inline constexpr vector_t<uint32_t> initial_vector<256, uint32_t>() { return {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19}; }
            template <> inline constexpr vector_t<uint64_t> initial_vector<384, uint64_t>() { return {0xcbbb9d5dc1059ed8, 0x629a292a367cd507, 0x9159015a3070dd17, 0x152fecd8f70e5939, 0x67332667ffc00b31, 0x8eb44a8768581511, 0xdb0c2e0d64f98fa7, 0x47b5481dbefa4fa4}; }
            template <> inline constexpr vector_t<uint64_t> initial_vector<512, uint64_t>() { return {0x6a09e667f3bcc908, 0xbb67ae8584caa73b, 0x3c6ef372fe94f82b, 0xa54ff53a5f1d36f1, 0x510e527fade682d1, 0x9b05688c2b3e6c1f, 0x1f83d9abfb41bd6b, 0x5be0cd19137e2179}; }
        }
        ...
        inline namespace context
        {
            ...
            static inline constexpr sha224_state_t create_sha224_state() { return {functions::initial_vector<224, uint32_t>(), {}, 0}; }
            static inline constexpr sha256_state_t create_sha256_state() { return {functions::initial_vector<256, uint32_t>(), {}, 0}; }
            static inline constexpr sha384_state_t create_sha384_state() { return {functions::initial_vector<384, uint64_t>(), {}, 0}; }
            static inline constexpr sha512_state_t create_sha512_state() { return {functions::initial_vector<512, uint64_t>(), {}, 0}; }

            static inline constexpr void update_vec_by_buf(sha224_state_t& input) { return functions::process_chunk<224, uint32_t>(input.vec, input.input); }
            static inline constexpr void update_vec_by_buf(sha256_state_t& input) { return functions::process_chunk<256, uint32_t>(input.vec, input.input); }
            static inline constexpr void update_vec_by_buf(sha384_state_t& input) { return functions::process_chunk<384, uint64_t>(input.vec, input.input); }
            static inline constexpr void update_vec_by_buf(sha512_state_t& input) { return functions::process_chunk<512, uint64_t>(input.vec, input.input); }

            template <class sha2_state_t = sha256_state_t>
    ...
    using impl::sha224_state_t;
    using impl::sha256_state_t;
    using impl::sha384_state_t;
    using impl::sha512_state_t;

    using impl::create_sha224_state;
    using impl::create_sha256_state;
    using impl::create_sha384_state;
    using impl::create_sha512_state;
    using impl::process_bytes;
    using impl::finalize_and_get_digest;
}
