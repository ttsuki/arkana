...
namespace arkana::camellia
{
    ...
    inline void process_blocks_ecb_avx2(void* dst, const void* src, size_t length, const key_vector_small_t& kv)
    {
        return avx2::process_blocks_ecb(dst, src, length, type_punning_cast<const avx2::key_vector_small_t&>(kv));
    }

    inline void process_blocks_ecb_avx2(void* dst, const void* src, size_t length, const key_vector_large_t& kv)
    {
        return avx2::process_blocks_ecb(dst, src, length, type_punning_cast<const avx2::key_vector_large_t&>(kv));
    }

    inline void process_bytes_ctr_avx2(void* dst, const void* src, size_t position, size_t length, const key_vector_small_t& kv, const ctr_vector_t& cv)
    {
        return avx2::process_bytes_ctr(dst, src, position, length, type_punning_cast<const avx2::key_vector_small_t&>(kv), type_punning_cast<const avx2::ctr_vector_t&>(cv));
    }

    inline void process_bytes_ctr_avx2(void* dst, const void* src, size_t position, size_t length, const key_vector_large_t& kv, const ctr_vector_t& cv)
    {
        return avx2::process_bytes_ctr(dst, src, position, length, type_punning_cast<const avx2::key_vector_large_t&>(kv), type_punning_cast<const avx2::ctr_vector_t&>(cv));
    }
    ...
    static inline std::unique_ptr<ctr_context_t> make_avx2_ctr_context(key_vector_t kv, ctr_vector_t cv)
    {
