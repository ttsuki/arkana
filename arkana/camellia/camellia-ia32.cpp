...
namespace arkana::camellia
{
    ...
    static inline std::unique_ptr<ecb_context_t> make_ia32_ecb_context(key_t k, enc_t e)
    {
        ...
        public:
            ...
            explicit ecb_context_impl_t(key_t key, enc_t enc) : key_vector_(ia32::impl::generate_key_vector(key, enc)) { }
            void process_blocks(void* dst, const void* src, size_t length) override { return ia32::impl::process_blocks_ecb(dst, src, length, key_vector_); }
        };
